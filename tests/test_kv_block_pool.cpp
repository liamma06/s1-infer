...
TEST_CASE("append spanning multiple blocks in one call") {
    ...
    TensorPtr out_k = pool.get_k(0);
    REQUIRE(out_k->shape()[1] == 5);
    for (size_t i = 0; i < 5; i++) {
        CHECK(out_k->at({0, i, 0}) == doctest::Approx(static_cast<scalar_t>(i + 1)));
    }
}
...
TEST_CASE("append across repeated calls continues filling a partial block correctly") {
    ...
    TensorPtr k1 = std::make_shared<Tensor>(std::vector<size_t>{1, 3, 1}, std::vector<scalar_t>{1, 2, 3});
    TensorPtr v1 = std::make_shared<Tensor>(std::vector<size_t>{1, 3, 1}, std::vector<scalar_t>{1, 2, 3});
    pool.append(0, k1, v1);

    TensorPtr k2 = std::make_shared<Tensor>(std::vector<size_t>{1, 3, 1}, std::vector<scalar_t>{4, 5, 6});
    TensorPtr v2 = std::make_shared<Tensor>(std::vector<size_t>{1, 3, 1}, std::vector<scalar_t>{4, 5, 6});
    pool.append(0, k2, v2);

    TensorPtr out_k = pool.get_k(0);
    REQUIRE(out_k->shape()[1] == 6);
...
TEST_CASE("remove frees blocks for reuse by another sequence") {
    ...
    TensorPtr k = std::make_shared<Tensor>(std::vector<size_t>{1, 2, 1}, std::vector<scalar_t>{1, 2});
    TensorPtr v = std::make_shared<Tensor>(std::vector<size_t>{1, 2, 1}, std::vector<scalar_t>{1, 2});
    ...
    CHECK(out_k->at({0, 0, 0}) == doctest::Approx(1.0f));
    CHECK(out_k->at({0, 1, 0}) == doctest::Approx(2.0f));
