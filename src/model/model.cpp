...
TensorPtr model_forward_quantized(
    ...
    for (size_t layer = 0; layer < 28; ++layer){
        ...
        x = layer_forward_quantized(
            x,
            weights.at(layer_prefix + "input_layernorm.weight"),
            quantized_weights.at(layer_prefix + "self_attn.q_proj.weight"),
            quantized_weights.at(layer_prefix + "self_attn.k_proj.weight"),
            quantized_weights.at(layer_prefix + "self_attn.v_proj.weight"),
            quantized_weights.at(layer_prefix + "self_attn.o_proj.weight"),
            weights.at(layer_prefix + "self_attn.q_norm.weight"),
            weights.at(layer_prefix + "self_attn.k_norm.weight"),
            weights.at(layer_prefix + "post_attention_layernorm.weight"),
            quantized_weights.at(layer_prefix + "mlp.gate_proj.weight"),
            quantized_weights.at(layer_prefix + "mlp.up_proj.weight"),
            quantized_weights.at(layer_prefix + "mlp.down_proj.weight"),
            caches[layer],
            sequence_id
        );
    }
    ...
    auto logits = matmul_quantized(output_norm, quantized_weights.at("lm_head.weight"));
...
GenerationResult generate_quantized(
    ...
    std::vector<int> token_ids = new_token_ids;
    std::vector<double> step_ms;
    size_t vocab_size = quantized_weights.at("lm_head.weight").cols;
    ...
    for (size_t i = 0; i < max_new_tokens; ++i) {
        auto step_start = std::chrono::steady_clock::now();
        auto logits = model_forward_quantized(next_input, weights, quantized_weights, caches, sequence_id, true);
        auto step_end = std::chrono::steady_clock::now();
        step_ms.push_back(std::chrono::duration<double, std::milli>(step_end - step_start).count());
